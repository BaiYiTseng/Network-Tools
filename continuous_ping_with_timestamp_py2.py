...
username = os.getenv('username')        # Python 3: os.getlogin()
os.chdir("C:\\Users\\" + username + "\\Desktop")

# Create a file name to store the output, default: new_icmp_log_dates.txt
filename = raw_input('Enter the name of the text file youd like the output to be saved as: ')
if not filename:
...
print_on_screen = raw_input('Would you like to print the output on the console as well?(Y/N) ')

destination_ip = raw_input('Please enter the destination IP address: ')

# Input monitor duration
duration = {}
print 'How long to monitor the ICMP response.'
duration['days'] = raw_input('\tHow many days: ')
duration['hours'] = raw_input('\tHow many hours: ')
duration['minutes'] = raw_input('\tHow many minutes: ')
duration['seconds'] = raw_input('\tHow many seconds: ')
