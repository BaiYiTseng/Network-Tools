...
serverPort = int(raw_input("Please enter the application port: "))
# protocol = input("Please enter the protocol: (TCP/UDP")
successfulInterval = int(raw_input("Please enter how often you would like to print successful connection: "))
monitoringDuration = int(raw_input("Please enter how long you would like to monitor in seconds: "))
file = open("monitoring_telnet_8.8.4.4.txt", "w")
startT = datetime.datetime.now()
initialT = startT

print("The output is saved in the following directory:", os.getcwd())
print("File name: monitoring_telnet_8.8.4.4.txt")
...
while True:
	try:
		...
		print("Connecting to %s:%d, Start time: %s, End time: %s, DeltaT: %d seconds, Total Duration: %d seconds \r" % 
		      (serverIP, serverPort,
		       str(startT.hour)+":"+str(startT.minute)+":"+str(startT.second),
		       str(currentT.hour)+":"+str(currentT.minute)+":"+str(currentT.second), 
		       deltaT, totalDuration,)),
